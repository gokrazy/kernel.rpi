...
func find(filename string) (string, error) {
	...
	return "", fmt.Errorf("could not find file %q (looked in .)", filename)
}
...
func main() {
	...
	cmd := exec.Command("go", "build", "-o", buildPath, "github.com/gokrazy/kernel.rpi/cmd/gokr-build-kernel")
	...
	if err := copyFile("vmlinuz", filepath.Join(tmp, "vmlinuz")); err != nil {
		log.Fatal(err)
	}

	dtbs, err := filepath.Glob(filepath.Join(tmp, "*.dtb"))
	if err != nil {
		log.Fatal(err)
	}
	for _, fn := range dtbs {
		if err := copyFile(filepath.Base(fn), filepath.Join(tmp, filepath.Base(fn))); err != nil {
			log.Fatal(err)
		}
	}
	...
	rm := exec.Command("rm", "-rf", filepath.Join("lib", "modules"))
	rm.Stdout = os.Stdout
	rm.Stderr = os.Stderr
	if err := rm.Run(); err != nil {
		log.Fatalf("%v: %v", rm.Args, err)
	}
	cp := exec.Command("cp", "-r", filepath.Join(tmp, "lib/modules"), "lib")
	cp.Stdout = os.Stdout
	cp.Stderr = os.Stderr
